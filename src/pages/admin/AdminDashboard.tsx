...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "lg:fixed lg:inset-y-0 lg:left-0 lg:z-50 bg-card border-r border-border flex flex-col transform transition-transform duration-300 ease-in-out",
        isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0",
        "w-64 lg:w-64 md:w-64"
      )}>
        <div className="p-4 md:p-6 border-b border-border flex-shrink-0">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-gradient-to-br from-primary to-gold-dark flex items-center justify-center">
            ...
            <div className="block">
              ...
              <span className="text-xs text-black">Admin Panel</span>
            </div>
          </Link>
        </div>
        ...
        <nav className="flex-1 p-2 md:p-4 space-y-2 overflow-y-auto">
          {navItems.map((item) => (
            ...
            >
              <item.icon className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0 text-black" />
        ...
        <div className="p-2 border-t border-border flex-shrink-0">
          <div className="flex items-center gap-2 mb-3 px-2">
            ...
            <div className="block">
              <p className="font-medium text-xs text-black">{user?.first_name || 'Admin'}</p>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto h-screen lg:ml-64">
        <Outlet />
      </main>
    </div>
  );
